...
test("completed recorded mission is operationally trustworthy", async ({
  page,
}) => {
  const pageErrors = [];
  ...
  await expect(readiness.getByText("ready", { exact: true })).toHaveCount(5, {
    timeout: 30_000,
  });

  const snapshotResponse = await page.request.get(
    "http://localhost:8000/api/runs/current/snapshot",
  );
  const snapshot = await snapshotResponse.json();
  const robotTopicCount = snapshot.topics.filter(
  ...
  await expect(
    page.locator(".topic-history-table tbody .status-pill"),
  ).toHaveCount(robotTopicCount);
  await page.locator(".technical summary").click();
  await expect(
    page.getByText(
      `${snapshot.topic_count} / ${snapshot.topic_count} topic summaries independently verified`,
    ),
  ).toBeVisible();
...
test("camera dropout remains legible after recovery", async ({ page }) => {
  test.skip(
    !process.env.EXPECT_DROPOUT,
    "requires the completed dropout Compose mission",
  );
  await page.goto("/");
...
test("state flow never leaves stale health behind", async ({ page }) => {
  ...
  await page.route("**/api/flink/summary", (route) =>
    route.fulfill({ json: { status: "available" } }),
  );
  await page.route("**/api/runs/current/snapshot", (route) =>
    route.fulfill({
      json: {
        run_id: null,
        run: null,
