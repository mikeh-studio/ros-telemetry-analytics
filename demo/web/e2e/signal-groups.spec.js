...
const metric = (topic, rate, enabled = true) => ({
  topic,
  robot_id: "robot-nav2-1",
  payload: {
    health_status: "healthy",
    status: "ok",
    mean_rate_hz: rate,
    expected_rate_hz: rate || 1,
    rate_monitoring_enabled: enabled,
  },
});
const base = {
  run_id: "nav2-fixture",
  ...
  topics: [
    metric("/scan", 5),
    metric("/odom", 30),
    metric("/amcl_pose", 0, false),
    metric("/_telemetry/gateway_health", 1),
    metric("/_telemetry/gateway_events", 0, false),
  ],
  anomalies: [],
  incident_history: [],
  observed_signals: [],
  ...
  completion: {},
  consumer_offsets: [],
  mission_duration_ms: 120000,
  mission_progress_ms: 12000,
...
async function openFixture(page, snapshot = base, readinessGate) {
  ...
  await page.goto("/");
  await expect(
    page.getByRole("region", { name: "Monitored Topics" }),
  ).toBeVisible();
}

test("Nav2 robot signals and event evidence stay separate on desktop and mobile", async ({
  page,
}, testInfo) => {
  const errors = [];
  ...
  await page.screenshot({
    path: testInfo.outputPath("signals-desktop.png"),
    fullPage: true,
  });

  const fault = {
    ...base,
    topics: base.topics.map((item) =>
      item.topic.endsWith("gateway_health")
        ? { ...item, payload: { ...item.payload, health_status: "degraded" } }
        : item,
    ),
    observed_signals: [
      {
  ...
  const notice = page.getByRole("link", {
    name: /Telemetry Pipeline: delivery needs attention/,
  });
  await expect(notice).toBeVisible();
  const eventCard = pipeline
    .locator("article")
    .filter({ hasText: "Gateway events" });
  ...
  await expect(pipeline).toHaveAttribute("open");
  await page.screenshot({
    path: testInfo.outputPath("pipeline-fault-desktop.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: testInfo.outputPath("signals-mobile.png"),
    fullPage: true,
  });
  expect(errors).toEqual([]);
});

test("recorded demo keeps all four robot signals and no fabricated gateway streams", async ({
  page,
}) => {
  await openFixture(page, {
    ...base,
    ...
    topic_count: 4,
    topics: [
      metric("/camera/image_raw", 30),
      metric("/imu/data", 100),
      metric("/odom", 20),
      metric("/diagnostics", 1),
    ],
  });
  const robot = page.getByRole("region", { name: "Monitored Topics" });
  await expect(robot.locator("tbody tr")).toHaveCount(4);
  await expect(robot).toContainText(
    "Image delivery supports the robot’s vision.",
  );
