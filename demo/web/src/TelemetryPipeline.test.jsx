...
const health = {
  ...
  robot_id: "r1",
  payload: { health_status: "healthy", mean_rate_hz: 1, expected_rate_hz: 1 },
};
const events = {
...
it("does not report silent event streams as a pipeline fault or fixed-rate stream", () => {
  ...
  expect(container.querySelector(".telemetry-pipeline")).not.toHaveAttribute(
    "open",
  );
  expect(screen.getByText("Monitoring")).toBeInTheDocument();
  expect(
    screen.getByText("No events observed in retained evidence."),
  ).toBeInTheDocument();
...
it("opens for a fault arriving after mount and can still be collapsed", () => {
  const { container, rerender } = render(
    <TelemetryPipeline topics={[health]} />,
  );
  rerender(
    <TelemetryPipeline
      topics={[{ ...health, payload: { health_status: "degraded" } }]}
    />,
  );
  expect(container.querySelector(".telemetry-pipeline")).toHaveAttribute(
    "open",
  );
...
it("shows the latest event by timestamp, its evidence, and severity without inventing current failure", () => {
  const observation = (timestamp, kind) => ({
    topic: events.topic,
    robot_id: "r1",
    metric_id: String(timestamp),
    stream_timestamp_ms: timestamp,
    payload: {
      attributes: { event_kind: kind, affected_topic: "/scan", count: 2 },
    },
  });
  const { container } = render(
    <TelemetryPipeline
      ...
      startMs={1000}
      signals={[
        observation(5000, "qos_incompatible"),
        observation(2000, "older_event"),
      ]}
    />,
  );
  expect(container.querySelector(".telemetry-pipeline")).toHaveAttribute(
  ...
  const { container } = render(
    <TelemetryPipeline
      ...
      signals={[
        {
          ...health,
          stream_timestamp_ms: 5000,
          payload: {
            attributes: {
              transport_state: "disconnected",
              last_transport_error: "KafkaTimeoutError",
            },
          },
        },
      ]}
