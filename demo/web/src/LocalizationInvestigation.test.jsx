...
import LocalizationInvestigation, {
  decisionText,
  pathFor,
  sampleAt,
} from "./LocalizationInvestigation";

const cases = ["detected", "missed", "false_alarm"].map((outcome, i) => ({
  case_id: String(i),
  outcome,
  start_ms: 10,
  end_ms: 20,
  duration_ms: 10,
  source_file: "recording",
  segment_id: 0,
  onset_lag_ms: i ? null : 5,
}));
const samples = [0, 10, 10, 20, 500, 510, 520].map((elapsed_ms, i) => ({
  elapsed_ms,
  sample_index: i,
  source_file: "recording",
  segment_id: 0,
  run_id: "run",
  break_before: i === 0 || i === 4,
  ground_truth_x: i,
  ground_truth_y: 0,
  estimated_x: i + 0.1,
  estimated_y: 0.1,
  particle_position_spread_m: 0.2,
  estimated_pose_jump_m: 0.1,
  detector_score: 0.5,
  position_error_m: 0.14,
  detector_failure: i === 2,
  label_failure: i > 0 && i < 4,
}));
const evaluation = {
  status: "available",
  summary: {
    thresholds: { particle_spread_warn_m: 0.4, pose_jump_warn_m: 0.5 },
  },
  investigation: {
    status: "available",
    evaluation_id: "version",
    configuration_id: "config",
    dataset: "Test recording",
    duration_ms: 520,
    recordings: ["recording"],
    cases,
  },
};
const interval = (case_id) => ({
  case_id,
  evaluation_id: "version",
  samples,
...
async function open() {
  const result = render(
    <LocalizationInvestigation evaluation={evaluation} apiUrl="" />,
  );
...
const installFetch = () =>
  vi.stubGlobal(
    ...
    vi.fn(async (url) =>
      response(
        interval(new URL(url, "http://localhost").searchParams.get("case_id")),
      ),
    ),
  );
...
it("loads event filters, keeps sample stepping exact at duplicate timestamps, and hides markers in gaps", async () => {
  ...
  fireEvent.change(screen.getByRole("slider", { name: "Playback time" }), {
    target: { value: "200" },
  });
...
it("plays, pauses, loops, and stops at the interval end without affecting mission playback", async () => {
  ...
  expect(
    screen.getByRole("button", { name: "Play interval" }),
  ).toBeInTheDocument();
...
it("does not replace selected event evidence when an older request finishes late", async () => {
  ...
  await screen.findByText(/This detector alert did not match/);
  await act(async () =>
    resolveOld(response({ ...interval("1"), sample_count: 999 })),
  );
  expect(screen.queryByText(/999 samples/)).not.toBeInTheDocument();
  expect(
    screen.getByText(/This detector alert did not match/),
  ).toBeInTheDocument();
});
...
it("clears stale evidence and exposes an API evaluation conflict", async () => {
  ...
  fetch.mockResolvedValueOnce({
    ok: false,
    json: async () => ({
      detail: "Evaluation changed. Refresh and select the event again.",
    }),
  });
  fireEvent.click(screen.getByRole("button", { name: "Next event" }));
  await waitFor(() =>
    expect(screen.getByRole("alert")).toHaveTextContent("Evaluation changed"),
  );
  expect(
    screen.queryByRole("button", { name: "Play interval" }),
...
it("does not claim thresholds stayed below their limits when evidence is missing", () => {
  const result = decisionText(
    ...
    [
      {
        elapsed_ms: 10,
        particle_position_spread_m: null,
        estimated_pose_jump_m: 0.1,
      },
    ],
    evaluation.summary.thresholds,
  );
...
it("breaks plot lines at gaps and source boundaries and returns no stale sample inside a gap", () => {
  const rows = [
    ...
    { ...samples[1], value: 2 },
    { ...samples[4], value: 3 },
    { ...samples[5], source_file: "other", value: 4 },
  ];
  expect(
    pathFor(
      rows,
  ...
  ).toBe("M0.00,1.00 L10.00,2.00 M500.00,3.00 M510.00,4.00");
...
it("matches the saved detector's strict threshold comparison", () => {
  const result = decisionText(
    ...
    [
      {
        elapsed_ms: 10,
        particle_position_spread_m: 0.4,
        estimated_pose_jump_m: 0.5,
      },
    ],
    evaluation.summary.thresholds,
  );
