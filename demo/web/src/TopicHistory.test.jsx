...
it("keeps Nav2 localization update-based and explains why signals matter", () => {
  const metric = { topic: "/amcl_pose", robot_id: "r1", payload: { rate_monitoring_enabled: false, mean_rate_hz: 0, expected_rate_hz: 1, health_status: "healthy" } };
  render(<TopicHistory topics={[metric]} startMs={0} signals={[{ ...metric, stream_timestamp_ms: 4200, payload: { attributes: { covariance: 0.25 } } }]} />);
  expect(screen.getByRole("region", { name: "Monitored Topics" })).toBeInTheDocument();
  expect(screen.getByText("The map pose supports navigation and localization.")).toBeInTheDocument();
  ...
  expect(screen.getByText(/This is an estimate, not ground truth/)).toBeInTheDocument();
  expect(screen.getByText("covariance")).toBeInTheDocument();
});

it("shows incident onset beside the affected robot signal", () => {
  render(<TopicHistory topics={[sample(13000, 0)]} startMs={1000} incidents={[{ anomaly_id: "gap", topic: "/odom", robot_id: "r1", status: "active", effective_start_stream_ms: 5000 }]} />);
  expect(screen.getByText("Unreliable since 00:04 into run")).toBeInTheDocument();
});

it("does not use a raw rate status as event-driven health", () => {
  render(<TopicHistory topics={[{ topic: "/diagnostics", payload: { rate_monitoring_enabled: false, status: "gap", mean_rate_hz: 0 } }]} />);
...
it("keeps camera sources distinct and omits generic purpose text", () => {
  const topics = ["/cam0/image_raw", "/cam1/image_raw", "/custom_a/data", "/custom_b/data"].map(topic => ({ topic, payload: { status: "healthy", mean_rate_hz: 20 } }));
  const { container } = render(<TopicHistory topics={topics} history={[]} startMs={0} durationMs={10000} />);
  expect(screen.getByRole("button", { name: "Camera 0", exact: true })).toBeInTheDocument();
  ...
  expect(screen.getByRole("button", { name: "Camera 0", exact: true })).toHaveAttribute("aria-pressed", "false");
  expect(screen.getByRole("button", { name: "/custom_a/data", exact: true })).toBeInTheDocument();
  expect(screen.getByRole("button", { name: "/custom_b/data", exact: true })).toBeInTheDocument();
  ...
  expect(screen.queryByText(/Message delivery indicates/)).not.toBeInTheDocument();
});
