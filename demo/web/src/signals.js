...
export const isEventDriven = (metric) =>
  metric.topic === "/_telemetry/gateway_events" ||
  metric.payload?.rate_monitoring_enabled === false;
...
export function latestObservation(signals, topic, robotId) {
  return signals
    .filter(
      (item) => item.topic === topic && (!robotId || item.robot_id === robotId),
    )
    .sort(
      (a, b) =>
        b.stream_timestamp_ms - a.stream_timestamp_ms ||
