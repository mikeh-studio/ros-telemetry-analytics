...
import { isEventDriven, signalLabel as label, signalPurpose, signalTime, latestObservation } from "./signals";
...
export default function TopicHistory({ topics = [], history = [], startMs, durationMs, incidents = [], signals = [], unavailable, live, completed }) {
  ...
  return <section ref={plotRoot} className="topic-workspace" aria-label="Monitored Topics">
    <div className="section-title compact"><h2>Monitored Topics</h2><span>{live ? "Live session" : "Recorded windows"} · 00:00–{time(duration)}</span></div>
    ...
    <p className="history-caption">Delivery health shows which topic data became unreliable. Rate charts: 10 s windows, 1 s slide; dashed line: expected; hollow dots: partial windows.</p>
    {!rows.length && <p className="empty-state">No topics observed for this run yet.</p>}
    <div className="topic-table-wrap"><table className="topic-history-table">
      <thead><tr><th scope="col">Topic</th><th scope="col">{completed ? "End state" : "Latest health"}</th><th scope="col">Rate history / update</th><th scope="col">Observed</th><th scope="col">Expected</th><th scope="col">Window max gap</th></tr></thead>
      ...
      const activeIncident = incidents.find((item) => item.topic === metric.topic && (!metric.robot_id || item.robot_id === metric.robot_id) && item.status === "active");
      const status = payload.health_status || (eventDriven ? activeIncident ? "degraded" : observation ? "observed" : "waiting" : payload.status) || "waiting";
      ...
      return <tr className={`topic-lane${selectedRow?.metric.topic === metric.topic ? " selected" : ""}`} key={metric.topic}>
        <td className="lane-identity" data-label="Topic"><div className="topic-name"><button className="topic-select" onClick={() => setSelected(metric.topic)} aria-pressed={selectedRow?.metric.topic === metric.topic}>{label(metric.topic)}</button></div>
          {label(metric.topic) !== metric.topic && <code>{metric.topic}</code>}{signalPurpose(metric.topic) && <p className="signal-purpose">{signalPurpose(metric.topic)}</p>}</td>
        <td data-label="Health"><span className={`status-pill ${unavailable ? "warn" : ["healthy", "ok"].includes(status) ? "ok" : ["observed", "waiting", "starting"].includes(status) ? "idle" : "warn"}`}>{unavailable ? "unavailable" : status}</span>{activeIncident && <span className="signal-onset">Unreliable since {signalTime(activeIncident.effective_start_stream_ms, startMs)}</span>}</td>
        <td className="lane-plot" data-label="Rate history">
          {eventDriven ? <div className="event-update"><strong>Last observed update</strong><span>{signalTime(observation?.stream_timestamp_ms, startMs)}</span><span>Updates as published · no rate target</span></div> : <><svg onPointerDown={(event) => { const rect = event.currentTarget.getBoundingClientRect(); setCursor(Math.min(duration, Math.round(Math.max(0, (event.clientX - rect.left - 34) / (plotWidth - 44) * duration) / 1000) * 1000)); }} viewBox={`0 0 ${plotWidth} 54}`} role="img" aria-label={`${metric.topic} message rate history, ${samples.length} recorded windows`}>
        ...
        <td className="lane-value" data-label="Observed"><strong>{eventDriven ? "Update-based" : rate(atCursor?.payload?.mean_rate_hz)}</strong><span>{eventDriven ? "Latest observation" : atCursor?.payload?.window_status === "partial" ? "Partial window" : atCursor ? cursor == null ? "Latest full window" : `At ${time(cursor)}` : "No sample"}</span></td>
        <td data-label="Expected">{eventDriven ? "No fixed rate" : rate(expected)}</td>
        <td data-label="Window max gap">{eventDriven ? "—" : Number.isFinite(atCursor?.payload?.max_inter_message_gap_s) ? `${Number(atCursor.payload.max_inter_message_gap_s.toFixed(3))} s` : "—"}</td>
      </tr>;
    })}</tbody></table></div>
    {selectedRow && <details className="selected-topic-detail" open={cursor != null || selected != null ? true : undefined}><summary>Inspect topic · {label(selectedRow.metric.topic)}</summary><div aria-live="polite">
      ...
      <p>{isEventDriven(selectedRow.metric) ? `Last observed update: ${signalTime(latestObservation(signals, selectedRow.metric.topic, selectedRow.metric.robot_id)?.stream_timestamp_ms, startMs)}. No fixed-rate expectation; silence alone is not a rate fault.` : cursor == null ? `Latest raw window: ${rate(selectedRow.metric.payload?.mean_rate_hz)}${selectedRow.metric.payload?.window_status === "partial" ? " (partial)" : ""}.` : selectedSample ? `Window ending ${time(selectedSample.t)}: ${rate(selectedSample.value)} (${selectedSample.item.payload?.window_status || "window"}).` : `No recorded window ends at ${time(cursor)}.`} {selectedIncidents.length ? `${selectedIncidents.length} incident(s) recorded for this topic. See incident explanation below.` : "No incidents recorded for this topic."}</p>
      {isEventDriven(selectedRow.metric) && <dl className="signal-attributes">{Object.entries(latestObservation(signals, selectedRow.metric.topic, selectedRow.metric.robot_id)?.payload?.attributes || {}).map(([key, value]) => <div key={key}><dt>{key.replaceAll("_", " ")}</dt><dd>{value == null ? "Unavailable" : typeof value === "object" ? JSON.stringify(value) : String(value)}</dd></div>)}</dl>}
      <p>Delivery health does not establish sensor accuracy. Missing windows are left disconnected. The inspection cursor does not seek or restart playback.</p>
    </div></details>}
  </section>;
}
