...
const rate = (value) =>
  Number.isFinite(value) ? `${Number(value.toFixed(2))} Hz` : "—";
...
export function topicSamples(history, topic, startMs, robotId) {
  ...
  for (const item of history) {
    if (
      item.topic !== topic ||
      (robotId && item.robot_id !== robotId) ||
      !Number.isFinite(item.window_end_ms)
    )
      continue;
    const key = `${item.robot_id}:${item.window_start_ms}:${item.window_end_ms}`;
    if (
      !windows.has(key) ||
      (item.revision || 0) > (windows.get(key).revision || 0)
    )
  ...
  return [...windows.values()]
    .map((item) => ({
      item,
      t: item.window_end_ms - startMs,
      value: item.payload?.mean_rate_hz,
    }))
  ...
  return samples
    .map((sample) => {
      if (
        ...
        sample.item.payload?.window_status === "partial"
      ) {
        previous = null;
        return "";
      }
      const connected =
        previous && sample.t - previous.t <= 1500 && sample.t > previous.t;
      previous = sample;
...
export default function TopicHistory({
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(([entry]) =>
      setPlotWidth(Math.max(120, entry.contentRect.width)),
    );
    ...
    () =>
      topics.map((metric) => ({
        ...
        samples: topicSamples(
          ...
          startMs,
          metric.robot_id,
        ).filter((sample) => sample.t <= duration),
      })),
    [topics, history, startMs, duration],
  );
  const selectedRow =
    rows.find(({ metric }) => metric.topic === selected) || rows[0];
  ...
  return (
    ...
    >
      ...
      {!rows.length && (
        <p className="empty-state">No topics observed for this run yet.</p>
      )}
      <div className="topic-table-wrap">
        <table className="topic-history-table">
          <thead>
            <tr>
          ...
          <tbody>
            {rows.map(({ metric, samples }) => {
              ...
              const activeIncident = incidents.find(
                (item) =>
                  item.topic === metric.topic &&
                  (!metric.robot_id || item.robot_id === metric.robot_id) &&
                  item.status === "active",
              );
              const status =
                ...
                "waiting";
              const expected =
                payload.rate_monitoring_enabled === false
                  ? null
                  : payload.expected_rate_hz;
              const maximum =
                Math.max(
                  1,
                  expected || 0,
              ...
              const atCursor =
                cursor == null
                  ? latestComplete || metric
                  : samples.find((sample) => Math.abs(sample.t - cursor) < 500)
                      ?.item;
              return (
                <tr
                  className={`topic-lane${selectedRow?.metric.topic === metric.topic ? " selected" : ""}`}
                  key={metric.topic}
                >
                  <td className="lane-identity" data-label="Topic">
                    <div className="topic-name">
                      <button
                        className="topic-select"
                        onClick={() => setSelected(metric.topic)}
                        aria-pressed={
                          selectedRow?.metric.topic === metric.topic
                        }
                      >
                        {label(metric.topic)}
                      </button>
                    </div>
                    {label(metric.topic) !== metric.topic && (
                      <code>{metric.topic}</code>
                    )}
                    {signalPurpose(metric.topic) && (
                      <p className="signal-purpose">
                        {signalPurpose(metric.topic)}
                      </p>
                    )}
                  </td>
                  <td data-label="Health">
                    ...
                    {activeIncident && (
                      <span className="signal-onset">
                        Unreliable since{" "}
                        {signalTime(
                          activeIncident.effective_start_stream_ms,
                          startMs,
                        )}
                      </span>
                    )}
                  ...
                  <td className="lane-plot" data-label="Rate history">
                    {eventDriven ? (
                      <div className="event-update">
                        <strong>Last observed update</strong>
                        <span>
                          {signalTime(
                            observation?.stream_timestamp_ms,
                            startMs,
                          )}
                        </span>
                        <span>Updates as published · no rate target</span>
                      </div>
                    ) : (
                      <>
                        <svg
                          ...
                          viewBox={`0 0 ${plotWidth} 54}`}
                          role="img"
                          aria-label={`${metric.topic} message rate history, ${samples.length} recorded windows`}
                        >
                          <path
                            d={`M34 10V34H${plotWidth - 10}`}
                            className="plot-axis"
                          />
                          {incidents
                            .filter(
                              (incident) => incident.topic === metric.topic,
                            )
                            .map((incident) => {
                              const start = Math.max(
                                0,
                                incident.effective_start_stream_ms - startMs,
                              );
                              const end = Math.min(
                                duration,
                                (incident.recovered_stream_ms ??
                              ...
                              return Number.isFinite(start) && end >= start ? (
                                <rect
                                  ...
                                  height="24"
                                  className="plot-incident"
                                >
                                  <title>
                                    {incident.condition_type} · {time(start)}–
                                    {time(end)}
                                  </title>
                                </rect>
                              ) : null;
                          ...
                          {Number.isFinite(expected) && (
                            <path
                              d={`M34 ${y(expected)}H${plotWidth - 10}`}
                              className="plot-expected"
                            />
                          )}
                          <path
                            d={historyPath(samples, x, y)}
                            className="plot-series"
                          />
                          {samples
                            .filter((sample) => Number.isFinite(sample.value))
                            .map((sample) => (
                              <circle
                                key={`${sample.item.robot_id}:${sample.t}`}
                                cx={x(sample.t)}
                                cy={y(sample.value)}
                                r={
                                  sample.item.payload?.window_status ===
                                  "partial"
                                    ? 2.7
                                    : 1.4
                                }
                              ...
                              >
                                <title>
                                  {time(sample.t)}: {rate(sample.value)} ·{" "}
                                  {sample.item.payload?.window_status ||
                                    "window"}
                                </title>
                              </circle>
                          ...
                          <text x="28" y="16" textAnchor="end">
                            {Number(maximum.toFixed(1))}
                          </text>
                          <text x="28" y="37" textAnchor="end">
                            0
                          </text>
                          {[0, 0.5, 1].map((fraction) => (
                            <text
                              key={fraction}
                              x={x(duration * fraction)}
                              y="50"
                              textAnchor={
                                fraction === 0
                                  ? "start"
                  ...
                  <td className="lane-value" data-label="Observed">
                    <strong>
                      {eventDriven
                        ? "Update-based"
                        : rate(atCursor?.payload?.mean_rate_hz)}
                    </strong>
                    <span>
                      {eventDriven
                        ? "Latest observation"
                        : atCursor?.payload?.window_status === "partial"
                          ? "Partial window"
                          : atCursor
                            ? cursor == null
                              ? "Latest full window"
      ...
      {selectedRow && (
        <details
          className="selected-topic-detail"
          open={cursor != null || selected != null ? true : undefined}
        >
          ...
          <div aria-live="polite">
            ...
            <p>
              {isEventDriven(selectedRow.metric)
                ...
                : cursor == null
                  ? `Latest raw window: ${rate(selectedRow.metric.payload?.mean_rate_hz)}${selectedRow.metric.payload?.window_status === "partial" ? " (partial)" : ""}.`
                  : selectedSample
                    ? `Window ending ${time(selectedSample.t)}: ${rate(selectedSample.value)} (${selectedSample.item.payload?.window_status || "window"}).`
                    : `No recorded window ends at ${time(cursor)}.`}{" "}
              {selectedIncidents.length
                ? `${selectedIncidents.length} incident(s) recorded for this topic. See incident explanation below.`
                : "No incidents recorded for this topic."}
            </p>
            {isEventDriven(selectedRow.metric) && (
              <dl className="signal-attributes">
                {Object.entries(
                  latestObservation(
                    signals,
                    selectedRow.metric.topic,
                    selectedRow.metric.robot_id,
                  )?.payload?.attributes || {},
                ).map(([key, value]) => (
                  <div key={key}>
            ...
            <p>
              Delivery health does not establish sensor accuracy. Missing
              windows are left disconnected. The inspection cursor does not seek
              or restart playback.
            </p>
          </div>
        </details>
